...
function Show-FunctionStatus {
  ...
  if ($stack.length -gt 3) {
    ...
    switch ($Level) {
      ...
      'Live' {
        Write-Verbose -Message "$Function is [LIVE]. Please report issues via GitHub or 'Orbit-Module@outlook.com'"
      }
      'Unmanaged' {
    ...
    Write-Verbose -Message "Need help? Online:  $global:OrbitHelpURLBase$Function`.md"
  }
} #Show-FunctionStatus
